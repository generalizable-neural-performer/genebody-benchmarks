...
if __name__ == '__main__':
    ...
    out_dir = os.path.join(log_dir, 'render_'+os.path.basename(checkpoint)[6:-3])
    ...
    with torch.no_grad():
        ...
        for i, samples in tqdm(enumerate(dataloader)):
            ...
            gt = images.masked_fill_(masks <= 0, 1).clamp(0, 1)
            x = preds.masked_fill_(pred_masks <= 0.55, 1).clamp(0, 1)
            ...
            preds = preds.cpu()
            preds.masked_fill_(pred_masks.cpu() <= 0.6, 0) # fill invalid with 0
