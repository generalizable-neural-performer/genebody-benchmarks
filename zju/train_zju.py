    ...
    for i in trange(1, 1+args.epoch):
        ...
        for samples in tqdm(dataloader):
            ...
            if step % (args.batch * 20) == 0:
                ...
                # RGB_texture = RGB_texture.detach().cpu().numpy()#, 0, 1) * 255).astype(np.uint8)
                # pred_masks = (pred_masks.detach().cpu().numpy() * 255).astype(np.uint8)
                # for k in range(args.batch):
                #     imageio.imwrite(f'./logs/zhuna/rgb_{k}.png', preds[k].transpose(1,2,0))
                #     imageio.imwrite(f'./logs/zhuna/txt_{k}.png', RGB_texture[k].transpose(1,2,0))
                #     imageio.imwrite(f'./logs/zhuna/msk_{k}.png', pred_masks[k].transpose(1,2,0))
                writer.add_scalar('train/loss', loss.item(), step)
                # tqdm.write('loss at step {}: {}'.format(step, loss.item()))
