...
def get_smpls(path, kp_idxs, gender='neutral', ext_scale=1.0, scale_to_ref=True,
    ...
    bones = np.concatenate(bones, axis=0).reshape(-1, 3)###pose params
    betas = np.concatenate(betas, axis=0)### shape params
    root_bones = np.concatenate(root_bones, axis=0)### Rh
    # note: this is in global space, but different in ZJU's system
    Tp = torch.FloatTensor(np.concatenate(root_locs, axis=0))

    # intentionally separate these for clarity rotate head direction
    ...
    smpl = SMPL(model_path=model_path, gender=gender, joint_mapper=process_spin.SMPL_JOINT_MAPPER)
    ...
    vertices = smpl(betas=betas.mean(0)[None],
             body_pose=dummy[:, 1:],
             global_orient=dummy[:, :1],
             pose2rot=False).vertices[0]

    # print(vertices.shape)
    ## Write to an .obj file
    outmesh_path = './hello_smpl_zju.obj'
    with open( outmesh_path, 'w') as fp:
        for v in vertices:
            fp.write( 'v %f %f %f\n' % ( v[0], v[1], v[2]) )

        # for f in smplx.f+1: # Faces are 1-based, not 0-based in obj files
        #     fp.write( 'f %d %d %d\n' %  (f[0], f[1], f[2]) )
...
def process_zju_data(data_path, subject='377', training_view=[0, 6, 12, 18],
    ...
    for i, (img_path, cam_idx) in enumerate(zip(img_paths, cam_idxs)):
        ...
        ###kp_idxs:[0, 0, 0, 0, 1, 1, 1, 1...]four cams img idxs
    ...
    for c in unique_cams:
        ...
        for h_ in range(H):## if curr pixel in all cams imgs is fore, then 0, else median
            for w_ in range(W): ## same cams have same bkgd
                vals = []
    ...
    cam_pose_vis(output_path='./test_zju_gl.obj', camera_poses=c2ws, cds='gl', pose_type='c2w')
    ...
    kp3d = kp3d.reshape(-1, 1, 24, 3).repeat(6, 1).reshape(-1, 24, 3)
    print(imgs.shape, kp3d.shape, c2ws.shape, focals.shape)
    skeletons = draw_skeletons_3d(imgs, kp3d,
                            c2ws=c2ws[view], H=H, W=W, focals=focals[view], centers=centers, skel_type=SMPLSkeleton)

    for i, skel in enumerate(skeletons):
        basedir = 'test_zju'
        imageio.imwrite(os.path.join(basedir, 'skel', f'{i:05d}.png'), skel)
                                                                        
    print(betas.shape, kp3d.shape, bones.shape, skts.shape, rest_pose.shape, vertices.shape, pose_scale)
    ## betas:shapes, kp3d:each joint rot reltive to parent joint in word coord,
    # bones:pose, skts:norm kp3d, rest_pose:reference pose, vertices:vertices, posescale:1
...
if __name__ == '__main__':
    ...
    process_spin.write_to_h5py(os.path.join(data_path, f"{subject}_{split}.h5"), data)
