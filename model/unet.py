...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #         imageio.imwrite(f'./logs/zhuna/{i}.png', x[i, :3].detach().cpu().numpy().transpose(1,2,0))
        x = self.up5(x, x1)
        # if test:
        #     import imageio
        #     import numpy as np
        #     for i in range(len(x)):
        #         imageio.imwrite(f'./logs/zhuna/out_{i}.png', (x[i, :3]).detach().cpu().numpy().transpose(1,2,0))
        #         mask = (x[i, 3].detach().cpu().numpy() > 0.6).astype(np.float32)
        #         imageio.imwrite(f'./logs/zhuna/mask_{i}.png', mask)
        return x
