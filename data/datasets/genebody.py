...
class GeneBodyDataset(torch.utils.data.Dataset):

    def __init__(self,data_folder_path, frame_num, size, use_mask, transforms, near_far_size, 
        ...
        self.file_path = os.path.join(data_folder_path, subject, 'smpl')
        tot_frame_num = len(os.listdir(self.file_path))
        self.ims = sorted(os.listdir(self.file_path))
        self.ims = [name[:-4]for name in self.ims]

        self.cams = sorted(os.listdir(os.path.join(data_folder_path, subject, 'image')))
        if self.is_train:
            self.start_idx = 0
        ...
        sum_tmp = 0
        for i in trange(len(self.ims)):
            verts, faces, rgbs = load_obj_mesh(os.path.join(self.data_folder_path, subject, 'smpl_color', self.ims[i]+'.obj'))
        ...
        param_path = os.path.join(self.data_folder_path, subject, 'annots.npy')
    ...
    def __getitem__(self, index, need_transform = True):
        ...
        img_path = os.path.join(self.file_path.replace('smpl', 'image'), f'{cam_id:02d}')
        ...
        msk_path = os.path.join(self.file_path.replace('smpl', 'mask'), f'{cam_id:02d}')
        ...
        if self.cropping:
            ...
            img = img * (img_mask[...,None] > 128).astype(np.uint8)
            ...
            img_mask = F.to_tensor(Image.fromarray((img_mask> 128).astype(np.uint8)))
        else:
            img = Image.fromarray(img)
            img_mask = Image.fromarray(((img_mask> 128).astype(np.float32)*255).astype(np.uint8))
