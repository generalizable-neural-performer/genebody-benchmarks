...
import torchvision
from .vsense import VsenseDataset

class VsenseTotalDataset(torch.utils.data.Dataset):

    def __init__(self, cfg, transforms, is_train = True):
        super(VsenseTotalDataset, self).__init__()
        ...
        for d in zip(self.dataset_paths,self.frame_nums,self.skips, self.near_far, Holes,cfg.DATASETS.IGNORE_FRAMES):
            ...
            if is_train:
                ...
                dataset = VsenseDataset(d[0],
                                d[1],
                                cfg.DATASETS.MASK,
                            transforms, d[3], 1,False, d[4], is_train=is_train, subject=cfg.DATASETS.SUBJECT, move_cam=cfg.DATASETS.MOVE_CAM)
