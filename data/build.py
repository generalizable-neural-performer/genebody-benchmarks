...
from .datasets import VsenseTotalDataset
from .transforms import build_transforms
from .collate_batch import static_collate

def build_dataset(cfg, transforms, dataset, is_train=True, use_mask = False, is_need_all_data = False):

    if is_train:
        datasets = dataset(cfg, transforms,is_train)
...
def make_data_loader(cfg, dataset=VsenseTotalDataset, is_train=True, is_need_all_data = False, is_center =False):
    
    batch_size = cfg.SOLVER.IMS_PER_BATCH

    transforms = build_transforms(cfg, is_train,is_center = is_center)
    datasets = build_dataset(cfg, transforms, dataset,  is_train, is_need_all_data)
    ...
    if is_train:
        ...
        data_loader = data.DataLoader(
            datasets, collate_fn=static_collate, batch_size=batch_size, shuffle=False, num_workers=num_workers
        )
