...
class Random_Transforms(object):
    ...
    def __call__(self, img, Ks = None, Ts = None,  mask = None):
        ...
        img = T.functional.rotate(img, angle = np.rad2deg(rotation), resample = Image.BICUBIC, center =(K[0,2],K[1,2]), fill=(0,))
        ...
        ROI = T.functional.rotate(ROI, angle = np.rad2deg(rotation), resample = Image.BICUBIC, center =(K[0,2],K[1,2]), fill=(0,))
        ...
        if mask is not None:
            # st()
            mask = T.functional.rotate(mask, angle = np.rad2deg(rotation), resample = Image.BICUBIC, center =(K[0,2],K[1,2]), fill=(0,))
