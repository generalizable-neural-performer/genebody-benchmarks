...
def fscore(dist1, dist2, threshold=0.001):
    ...
    print(f'p1 {precision_1} p2 {precision_2}')
...
if __name__ == '__main__':
    ...
    NORMAL = []
    HOFF = []
    for mesh_name in os.listdir(pred_mesh_dir)[::30]:
        frame_id = int(mesh_name.split('.')[0])
        pred_mesh_path = os.path.join(pred_mesh_dir, mesh_name)
        pred_pcd = o3d.io.read_point_cloud(pred_mesh_path)
        gt_mesh_path = os.path.join(gt_mesh_dir, f'{(frame_id):06d}.ply') # NOTE
        print(pred_mesh_path, gt_mesh_path)
        ...
        FSCORE.append(fscore(chamfer1, chamfer2, threshold=10.0))
        NORMAL.append(np.mean(chamfer_normal))
        HOFF.append(np.max(chamfer1))
        break # NOTE: only test the first frame
