...
from core.utils.skeleton_utils import CMUSkeleton, SMPLSkeleton, SMPLXSkeleton, smpl_rest_pose, get_smpl_l2ws, get_per_joint_coords
...
def config_parser():
    ...
    # nerf config
    parser.add_argument('--nerf_args', type=str, required=True, default='logs/surreal_model/args.txt',
...
def load_render_data(args, nerf_args, poseopt_layer=None, opt_framecode=True):
    ...
    c2ws, focals, centers = dd.io.load(data_h5, cam_keys)
    ...
    if args.render_type in ['retarget', 'mesh']:
        ...
        is_ghr = 'genebody' in args.dataset
        if is_neuralbody:
            import h5py
    ...
    #print(gt_paths.shape)  (3600, 262144, 3)
    ...
    ##TODO: add centers here(for genebody centers)
...
def init_catalog(args, n_bullet=10):
    ...
    # GHR
    ghr_subjects = ['zhuna', 'abror', 'amaris', 'arslan', 'dannier', 'fuzhizhi2', 'huajiangtao5',
      'joseph_matanda',  'mahaoran',  'quyuanning', 'soufianou_boubacar_moumouni',  'wuwenyan',  'zhanghongwei',
      'ahha', 'anastasia', 'barlas',  'dilshod',  'gaoxing' , 'ivan', 'kamal_ejaz', 'maria', 'rivera', 'sunyuxing',
        'xujiarui',  'zhangzixiao', 'zhuna2', 'alejandro', 'barry', 'fenghaohan', 'jinyutong', 'kemal','natacha',
    'shchyerbina_oleksandr', 'Tichinah_jervier', 'yaoqibin', 'zhengxin', 'zhuxuezhi', 'amanda', 'aosilan', 'camilo', 'fuzhizhi',
    'huajiangtao3', 'jinyutong2', 'lihongyun',  'songyujie',  'wangxiang', 'zhanghao',  'zhonglantai']
    # TODO: hard-coded: 6 views
    ghr_idxs = np.arange(75 * 48)
    ghr_dict = lambda subject: {'data_h5': f'data/genebody/{subject}_test.h5',
                               'val': set_dict(ghr_idxs, length=1, skip=1)}
    ...
    RenderCatalog['genebody'] = {
        f'{subject}': ghr_dict(subject) for subject in ghr_subjects
    }
...
def load_retarget(pose_h5, c2ws, focals, rest_pose, pose_keys,
                  ...
                  undo_rot=False, is_neuralbody=False, is_ghr=False):
    ...
    if refined is None:
        if not is_surreal and not is_neuralbody and not is_ghr:
            ...
            kps = kps.reshape(-1, 1, 22, 3).repeat(6, 1).reshape(-1, 22, 3)
            bones = bones.reshape(-1, 1, 24, 3).repeat(6, 1).reshape(-1, 24, 3)
        elif is_ghr:
            ### may wrong when some subjects lacked cams(may not have 48 cams)
            kps, bones = dd.io.load(pose_h5, pose_keys)
            kps = kps.reshape(-1, 1, 22, 3).repeat(48, 1).reshape(-1, 22, 3)
            bones = bones.reshape(-1, 1, 24, 3).repeat(48, 1).reshape(-1, 24, 3)
        else:
            kps, bones = dd.io.load(pose_h5, pose_keys)
...
def evaluate_metric(rgbs, accs, bboxes, gt_dict, basedir, score_dicr_all=None, score_index=''):
    '''
    ...
    for i, (rgb, acc, bbox, gt_path) in enumerate(zip(rgbs, accs, bboxes, gt_paths)):
        ...
        if is_gt_paths:
            ...
            # print(gt_path.shape, gt_paths.shape) (262144, 3) (3600, 262144, 3)
            #(256, 256, 3)
    ...
    np.save(os.path.join(basedir, 'scores'+ score_index + '.npy'), score_dict, allow_pickle=True)

    with open(os.path.join(basedir, 'score_final' + score_index + '.txt'), 'w') as f:
        for k in score_dict:
            avg = np.mean(score_dict[k])
            f.write(f'{k}: {avg}\n')
    if score_dicr_all is not None:
        score_dicr_all.append(score_dict)
...
def run_render():
    ...
    tensor_data['render_poses'] = tensor_data['render_poses']
...
def run_render_genebody():
    ...
    score_dict_all = []
    score = {'psnr': [], 'ssim': [], 'fg_psnr': [], 'fg_ssim': []}
    ni = 450
    for start in range(0, len(tensor_data['render_poses']), ni):

        tensor_part = {}
        for k in tensor_data:

            if tensor_data[k] is None:
                tensor_part[k] = None
            elif k == 'hwf':
                tensor_part[k] = (tensor_data[k][0], tensor_data[k][1], tensor_data[k][2][start:start + ni])
            else:
                tensor_part[k] = tensor_data[k][start:start + ni]
        gt_part = {}
        for k in gt_dict:
            if type(gt_dict[k]) == bool:
                gt_part[k] = gt_dict[k]
            elif k == 'bg_imgs':
                gt_part[k] = gt_dict[k]
            else:
                gt_part[k] = gt_dict[k][start:start + ni]

        # print(tensor_data['kp'].shape)
        basedir = os.path.join(args.outputdir, args.runname)
        os.makedirs(basedir, exist_ok=True)
        if args.render_type == 'mesh':
            render_mesh(basedir, render_kwargs, tensor_part, res=args.mesh_res, chunk=nerf_args.chunk)
            return
        ...
        if gt_dict['gt_paths'] is not None:
            if args.eval:
                evaluate_metric(rgbs, accs, bboxes, gt_part, basedir, score_dicr_all=score_dict_all, score_index=str(start))
            if args.save_gt: 
                ...
                if gt_dict['is_gt_paths']:
                    ...
                    os.makedirs(os.path.join(basedir, 'gt'), exist_ok=True)
                    for i in range(ni):
                        gt_img = gt_paths[i + start]
                        gt_img = gt_img.reshape((512, 512, 3))
                        imageio.imwrite(os.path.join(basedir, 'gt', f'{i + start:05d}.png'), gt_img)
        ...
        if 'genebody' in args.dataset:
            ...
            skel_type = SMPLSkeleton
        skeletons = draw_skeletons_3d(rgbs, np.array(tensor_part['kp'].cpu()),
                                    np.array(tensor_part['render_poses'].cpu()),
                                    *tensor_part['hwf'],centers=np.array(tensor_part['centers'].cpu()),
                                    skel_type=skel_type)
        ...
        for i, (rgb, acc, skel) in enumerate(zip(rgbs, accs, skeletons)):
            imageio.imwrite(os.path.join(basedir, 'image', f'{i + start:05d}.png'), rgb)
            imageio.imwrite(os.path.join(basedir, 'acc', f'{i + start:05d}.png'), acc)
            imageio.imwrite(os.path.join(basedir, 'skel', f'{i + start:05d}.png'), skel)
        np.save(os.path.join(basedir, 'bboxes_%d.npy' % start), bboxes, allow_pickle=True)
        imageio.mimwrite(os.path.join(basedir, "render_rgb_%d.mp4" % start), rgbs, fps=args.fps)

    with open(os.path.join(basedir, 'score_final_all.txt'), 'w') as f:
        for s in score_dict_all:
            for key in s:
                score[key] += s[key]
        for k in score:
            avg = np.mean(score[k])
