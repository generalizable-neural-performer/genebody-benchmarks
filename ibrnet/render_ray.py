...
from ibrnet.sample_ray import vh
...
def sample_along_camera_ray(ray_o, ray_d, depth_range,
    ...
    end = ray_o + ray_d * (far_depth_value - near_depth_value)
    pts = z_vals.unsqueeze(2) * ray_d + ray_o       # [N_rays, N_samples, 3]
    N_rays, N_samples = pts.shape[:2]

    if mesh_param is not None:
        center = mesh_param['center'].cuda().float()
        spatial_freq = mesh_param['spatial_freq'].cuda().float()
        pts_nml = ((pts - center) * spatial_freq / (512 / 2))

    if mesh_param is not None:
        return pts_nml, z_vals, pts
...
def raw2outputs(raw, z_vals, mask, white_bkgd=False):
    ...
    # sigma2alpha = lambda sigma, dists: 1. - torch.exp(-F.relu(sigma))
...
def render_rays(ray_batch,
    ...
    if 'mesh_param' in ray_batch.keys():
        pts, z_vals, pts2project = sample_along_camera_ray(ray_o=ray_batch['ray_o'],
                                            ray_d=ray_batch['ray_d'],
                                            depth_range=ray_batch['depth_range'],
                                            N_samples=N_samples, inv_uniform=inv_uniform, det=det,
                                            mesh_param=ray_batch['mesh_param'] if 'mesh_param' in ray_batch.keys() else None,
                                            ray_batch=ray_batch)
    else:
    ...
    rgb_feat, ray_diff, mask = projector.compute(pts, ray_batch['camera'],
                                                 ...
                                                 featmaps=featmaps[0],
                                                  ori_pts=pts2project)  # [N_rays, N_samples, N_views, x]
    ...
    if N_importance > 0:
        ...
        fine_Nrays, fine_Nsamples = pts.shape[:2]

        if 'mesh_param' in ray_batch.keys():
            mesh_param = ray_batch['mesh_param']
            center = mesh_param['center'].cuda().float()
            spatial_freq = mesh_param['spatial_freq'].cuda().float()
            pts2project = pts.clone()
            pts = ((pts - center) * spatial_freq / (512 / 2))
