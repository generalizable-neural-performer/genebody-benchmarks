...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        try:
            self.mesh_param = data['mesh_param']
        except:
    ...
    def get_all(self):
        ret = {'ray_o': self.rays_o.cuda(),
               ...
               'mesh_param': self.mesh_param if self.mesh_param is not None else None,
        }
        return ret
...
def index(feat, uv, mode='bilinear'):
    ...
    '''
    if len(feat.shape) == 3:
        feat = feat.unsqueeze(1) # BHW->B1HW
    ...
    # else:
    samples = torch.nn.functional.grid_sample(feat, uv, mode=mode, align_corners=True)
...
def inside_pts_vh(pts, masks, calibs, persps=None):
    xyz = perspective(pts.permute((1,0))[None,...].expand([calibs.shape[0],-1,-1]), persps, calibs)
    xy = xyz[:, :2, :]
    if persps is not None:
        xy = xy / torch.tensor([[[512],[512]]], \
...
def vh(pts, masks, calibs, persps):
    inside = inside_pts_vh(pts, masks, calibs, persps)
    return inside